...
from models.UCFN import build_UCFN
...
def parser_args():
    ...
    parser = argparse.ArgumentParser(description='UCFN')
    parser.add_argument('--dataname', help='dataname', default='intentonomy')
    parser.add_argument('--device_id', help='decive number', default=0)
    ...
    parser.add_argument('--resume', default="./checkpoint/checkpoint.tar", type=str, metavar='PATH',
                        help='path to latest checkpoint (default: none)')
    
    parser.add_argument('--resume_omit', default=[], type=str, nargs='*')
    parser.add_argument('-e', '--evaluate', default= 0, dest='evaluate', action='store_true',
...
def main_worker(args, logger):
    ...
    # build model
    model = build_UCFN(args)

    model = model.cuda()
    ema_m = ModelEma(model, args.ema_decay) # 0.9997
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.device_id], broadcast_buffers=False,find_unused_parameters=True)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            checkpoint = torch.load(args.resume, map_location=torch.device(args.device_id))
...
def train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger):
    ...
    for i, (images, target) in enumerate(stable(train_loader, args.seed+epoch)):
        ...
        labels_all = dict()
        labels_all[0] = labels
        labels_all[1] = labels_15
        labels_all[2] = labels_9

        # compute output
        with torch.cuda.amp.autocast(enabled=args.amp):
            evidence_a, loss_fine, loss_middle, loss_coarse = model(images, labels_all, epoch)
            output_sm = nn.functional.sigmoid(evidence_a[0])
            com_loss = compareLoss(loss_fine, loss_middle, loss_coarse)
            loss = 1*loss_fine+0.3*com_loss
        ...
        middle_l = loss_middle
        middle_loss.append(middle_l.item())
        coarse_l = loss_coarse
...
def validate(val_loader, model, criterion, args, logger):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # compute output
            with torch.cuda.amp.autocast(enabled=args.amp):
                evidence_a, loss_fine, loss_middle, loss_coarse = model(images, labels_all, 100)
                loss = loss_fine
                if args.loss_dev > 0:
                    loss *= args.loss_dev
                output_sm = nn.functional.sigmoid(evidence_a[0])
...
def compareLoss(loss_fine, loss_middle, loss_coarse):
    stacked_losses = torch.stack([loss_fine, loss_middle, loss_coarse])
    loss, _ = torch.max(stacked_losses, dim=0)
    return loss
