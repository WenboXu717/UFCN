...
from data_utils.get_dataset_new import CLASS_9, CLASS_15, CLASS_30, CLASS_18
import numpy as np
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=0,
        ...
        self.pro_m_t = nn.Linear(49, 56)  # 28 = number of fine-grained category
        self.att = nn.Sigmoid()
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False,dataname="intentonomy"):
        ...
        self.hier_class = [56,30,18]    # numbers of category on different levels
    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:
            ...
            if curr_layer == 0:
                for i in range(len(CLASS_30)):
                    length_value = len(CLASS_30[str(i)])
                    a = CLASS_30[str(i)]
                ...
                for i in range(len(CLASS_18)):
                    length_value = len(CLASS_18[str(i)])
                    a = CLASS_18[str(i)]
                    inter = output_inter[a[0]]
                    if length_value != 1:
                        for j in range(length_value - 1):
